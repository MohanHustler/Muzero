...
import BatchListItem from '../components/batch_list_item';
...
const CreateBatchTutor: FunctionComponent<Props> = ({ authUser, classes }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const studentsList = await fetchTutorStudentsList();
  ...
  const removeBatchItem = async (index: number) => {
    ...
    const batch = clonedBatches[index];

    clonedBatches.splice(index, 1);
    setBatches(clonedBatches);
    setOpenConfirmationModal(false);
    try {
      await deleteTutorBatch(batch);
    } catch (error) {
      exceptionTracker(error.response?.data.message);
      if (error.response?.status === 401) {
        setRedirectTo('/login');
  ...
  const updateBatch = async () => {
    ...
    try {
      ...
      await updateTutorBatch(updatedBatch);
      clearBatchDetails();
    } catch (error) {
      exceptionTracker(error.response?.data.message);
      if (error.response?.status === 401) {
...
export default connect(mapStateToProps)(
  withStyles(batchStyles)(CreateBatchTutor)
);
